"""
PCN Trainer Implementation - FIXED VERSION

This version fixes the critical data leakage issue in the test method.
The original implementation uses true labels during test inference, which
artificially inflates accuracy to ~99.9%. This version properly evaluates
the model without using labels during inference.
"""
...
class PCNTrainer:
    ...
    def train_batch(
        ...
        # INFERENCE PHASE - Update latents (WITH LABELS - correct for training)
    ...
    def test(
        ...
        """
        Test the model and compute accuracies WITHOUT using labels during inference.
        
        This is the FIXED version that properly evaluates the model's generalization
        ability without data leakage.
        ...
        for x_batch, y_batch in tqdm(test_loader):
            ...
            # Run UNSUPERVISED inference (no label information)
            with autocast(device_type='cuda'):
                for t in range(1, self.T_infer + 1):
                    # Compute only unsupervised errors
                    errors, gain_modulated_errors = self.model.compute_errors(inputs_latents)
                    
                    # Update latents using ONLY unsupervised gradients
                    # Note: We don't include supervised error for the top layer
                    for l in range(1, self.model.L + 1):
                        if l < self.model.L:
                            # Lower layers: use error from layer above
                            grad_Xl = errors[l] - gain_modulated_errors[l-1] @ weights[l-1]
                        else:
                            # Top layer: no supervised error, only feedback from below
                            grad_Xl = -gain_modulated_errors[l-1] @ weights[l-1]
                        
                        inputs_latents[l] -= self.eta_infer * grad_Xl
            
            # After unsupervised inference, compute predictions
            logits = self.model.readout(inputs_latents[-1])
            
            # NOW we can use labels to compute accuracy (but not during inference!)
            # Top-1 accuracy
    ...
    def test_with_labels(
        ...
        """
        Test the model using the ORIGINAL (flawed) method that uses labels during inference.
        
        This method is included for comparison purposes to demonstrate the data leakage.
        DO NOT use this for actual evaluation!
        
        Args:
            test_loader: DataLoader for test data
            
        Returns:
            top1_acc: Top-1 accuracy (inflated due to label leakage)
            top3_acc: Top-3 accuracy (inflated due to label leakage)
        """
        self.model.eval()
        ...
        for x_batch, y_batch in tqdm(test_loader):
            ...
            # Run inference WITH LABELS (the flawed approach)
            with autocast(device_type='cuda'):
                for t in range(1, self.T_infer + 1):
                    errors, gain_modulated_errors = self.model.compute_errors(inputs_latents)
                    y_hat = self.model.readout(inputs_latents[-1])
                    eps_sup = y_hat - y_batch  # USING TRUE LABELS!
                    eps_L = eps_sup @ weights[-1]
                    errors_extended = errors + [eps_L]
